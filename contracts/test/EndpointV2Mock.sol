...
import {IOAppCore} from "../vendor/layerzerolabs/oapp/interfaces/IOAppCore.sol";
import {
    MessagingFee,
    MessagingParams,
    MessagingReceipt,
...
import {
    Errors
} from "@layerzerolabs/lz-evm-protocol-v2/contracts/libs/Errors.sol";
import {GUID} from "@layerzerolabs/lz-evm-protocol-v2/contracts/libs/GUID.sol";
import {
    Transfer
} from "@layerzerolabs/lz-evm-protocol-v2/contracts/libs/Transfer.sol";
import {
    MessagingChannel
} from "@layerzerolabs/lz-evm-protocol-v2/contracts/MessagingChannel.sol";
import {
    MessagingComposer
} from "@layerzerolabs/lz-evm-protocol-v2/contracts/MessagingComposer.sol";
import {
    MessageLibManager
} from "@layerzerolabs/lz-evm-protocol-v2/contracts/MessageLibManager.sol";
import {
    MessagingContext
} from "@layerzerolabs/lz-evm-protocol-v2/contracts/MessagingContext.sol";
    ...
    MessagingContext
{
    ...
    function quote(
        ...
        uint64 nonce = outboundNonce[_sender][_params.dstEid][
            _params.receiver
        ] + 1;
        ...
        returns (MessagingReceipt memory)
    {
        ...
        // send message
        (MessagingReceipt memory receipt, address _sendLibrary) = _send(
            msg.sender,
            _params
        );
        ...
        /// @dev (MOCK) dont pay native fee to keep msg.value
        // _payNative(
        ...
        uint64 nonce = lazyInboundNonce[receiver][eid][sender];

        /// @dev (MOCK) Calls the dst endpoint
        _mockLzReceiveAfterSend(
            Origin(eid, sender, nonce),
            receiver,
            receipt.guid,
            _params.message
        );
    ...
    function _mockLzReceiveAfterSend(
        Origin memory _origin,
        address _receiver,
        bytes32 _guid,
        bytes memory _message
    ) internal {
        bytes32 dstEndpointBytes32 = IOAppCore(_receiver).peers(eid);
        address dstEndpoint = address(uint160(uint256(dstEndpointBytes32)));

        /**
         * @dev (MOCK) Override origin sender to dst endpoint.
         * Conceptually this isnt right but it is to bypass OnlyPeers()
         * when peers(srcEid) is set to dstEndpoint.
         */
        Origin memory originHack = Origin(
            eid,
            dstEndpointBytes32,
            _origin.nonce
        );
        ILayerZeroEndpointV2(dstEndpoint).verify(
            originHack,
            _receiver,
            keccak256(abi.encodePacked(_guid, _message))
        );

        ILayerZeroEndpointV2(dstEndpoint).lzReceive(
            originHack,
            _receiver,
            _guid,
            _message,
